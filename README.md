...
단위 테스트 38건. `./gradlew test`로 실행한다.

| 클래스 | 건수 | 다루는 것 |
|---|---|---|
| `RealtimeArrivalTest` | 18 | 도착 메시지 변환. 전역 접두, 역명 괄호 부제, `barvlDt` 우선순위, 남은 도착 시간(초) |
...
**Compose UI 테스트가 없다.** 단위 테스트 38건은 모두 JVM에서 도는 것이고
`androidTest` 소스셋 자체가 없다. 화면 상태 전이는 검증되지 않았다.

**위젯이 크기와 무관하게 3개 고정이다.** Glance `SizeMode.Responsive`를 쓰지 않아
2×2로 줄여도 3개를 그리려 한다.

**도착 알림이 폴링 구조다.** 예상 도착 시각 1분 전에 워커를 깨워 30초~2분 간격으로
실시간 도착 API를 다시 조회한다. 도착 시각이 바뀌어도 따라간다는 장점이 있지만
대가가 넷이다. WorkManager의 비포그라운드 실행 한도는 약 10분인데 워커는 1시간을
감시하려 하고, 중단 후 재실행되면 감시 시작 시각이 초기화된다. 기기에서 잰 워커
실행 시간은 2분 31초와 7분 31초였다. 뒤엣것은 한 정거장 뒤 열차에 5분 전 알림을 건
평범한 조합인데 **10분 한도의 75%**까지 갔다. Doze와 앱 대기 버킷에는 대응하지
않는다. 알람 1건이 최대 120회를 호출해 일일 한도 1,000건의 12%를
쓴다. 폴링 간격이 30초라 "1분 전"은 최대 30초 어긋난다. 예상 시각에 한 번만 깨어나
확인하고 어긋났으면 다시 예약하는 구조가 대안이지만, 재예약 로직과 도착 시각 급변
대응이 필요해 이번에는 두었다.

**알림 권한을 나중에 끄면 예약이 조용히 사라진다.** 워커가 발화 직전에 권한을
확인하고 없으면 그냥 돌아가는데, 호출부는 그대로 예약을 비활성으로 내리고 끝낸다.

**실시간 API 잠금이 풀리지 않는다.** 연속 3회 실패하면 `isApiLocked`가 서는데
`false`로 되돌리는 코드가 없어 프로세스가 죽을 때까지 잠긴 채로 남는다.

**알림 탭에서 같은 역·같은 종착역의 두 열차가 구분되지 않는다.** 한 줄에 역 이름과
"{종착}행 · 도착 N분 전 알림"만 표시하고 열차를 구분하는 값이 없다. 2호선 내선처럼
연속한 두 대의 종착역이 같으면 두 줄이 같아진다. 예약 당시 도착 예정 시각을 저장하면
되지만 Room 마이그레이션이 생긴다.

**알람 전용 테스트가 없다.** `SubwayAlarmManager`, `SubwayAlarmWorker`,
`NotificationHelper`는 단위 테스트 없이 기기에서 손으로 확인했다.

**발화한 알람 행이 지워지지 않는다.** `isActive = 0`으로 내려갈 뿐이고
`deleteAlarm` 쿼리는 정의만 되어 있다. 화면에는 영향이 없지만 행은 계속 쌓인다.

**도착 카드가 3대까지만 그린다.** 알람을 열차별로 걸 수 있게 됐지만
`arrivals.take(3)` 뒤의 열차는 화면에 없어 예약할 수 없다.
