...
class StationRepository @Inject constructor(
    ...
    @Named("seoulTimetableApiKey") private val seoulTimetableApiKey: String,
    @Named("dataGoKrServiceKey") private val dataGoKrServiceKey: String,
    ...
    suspend fun getRealtimeArrivalInfo(stationName: String): List<RealtimeArrival> {
        ...
        return runCatching {
            withContext(Dispatchers.IO) { 
                val response = realtimeApi.getStationArrivalInfo(seoulOpenApiKey, normalizedName)
    ...
    suspend fun getStationTimeTable(stationName: String, lineNumber: String, isWeekend: Boolean): Pair<com.jonghyeok.ezegot.api.TimeTableResponse?, com.jonghyeok.ezegot.api.TimeTableResponse?> =
        withContext(Dispatchers.IO) {
            val stationsResponse = runCatching { stationInfoApi.getStations(seoulOpenApiKey) }.getOrNull()
            ...
            if (!frCode.isNullOrEmpty()) {
                ...
                up = runCatching { stationInfoApi.getStationTimeTable(seoulTimetableApiKey, frCode, weekCode, "1") }.getOrNull()
                down = runCatching { stationInfoApi.getStationTimeTable(seoulTimetableApiKey, frCode, weekCode, "2") }.getOrNull()
            ...
            if (upEmpty && downEmpty) {
                val tagoListRes = runCatching { extendedApi.getTagoStationList(dataGoKrServiceKey, cleanName) }.getOrNull()
                ...
                if (nodeId != null) {
                    ...
                    val tagoUp   = fetchTagoTimeTable(nodeId!!, tagoDayCode, "U", dataGoKrServiceKey, extendedApi)
                    val tagoDown = fetchTagoTimeTable(nodeId!!, tagoDayCode, "D", dataGoKrServiceKey, extendedApi)
